...
class Test_ParameterConfigInline:
	...
	[0, 1, 2]
	>>> try_catch(lambda: test_get_parameter('test', {'test': '', 'test type': 'unknown'}), 'ConfigError', 'Invalid parameter type')
	caught
	>>> try_catch(lambda: test_get_parameter('test', {'test': 'range(', 'test type': 'expr'}), 'ConfigError', 'Invalid parameter value')
	caught
	...
	['1', '2', '3', '4']
	>>> try_catch(lambda: test_get_parameter('A', {'(A,B)': '1 2 3 4 5', '(A, B) type': 'unknown'}), 'ConfigError', 'Invalid tuple type')
	caught
	"""
