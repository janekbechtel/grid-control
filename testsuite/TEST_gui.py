...
class Test_GUI:
	...
	>>> try_catch(lambda: gui.display_workflow(workflow), 'AbstractError', 'is an abstract function')
	caught
	"""
...
class Test_Report:
	...
	>>> try_catch(lambda: Report.create_instance('Report', workflow.job_manager.job_db).show_report(workflow.job_manager.job_db), 'AbstractError', 'is an abstract function')
	caught

	>>> test_report(workflow.job_manager.job_db)
	['REPORT SUMMARY:                                                  ']

	>>> test_report(workflow.job_manager.job_db, workflow.task)
