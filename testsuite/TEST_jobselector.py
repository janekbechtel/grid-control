...
def fmtJobObj(jobnum, jobobj):
	...
	return '%02d | state: %-9s | attempt: %d | changed: %s | gc_id: %-17s | dest: %s' % (jobnum,
		Job.enum2str(jobobj.state), jobobj.attempt, changed_str, jobobj.gc_id, jobobj.get('dest'))

class Test_JobSelector:
	"""
	>>> print(str.join('\\n', imap(lambda x: fmtJobObj(*x), jobinfo_list)))
	00 | state: DONE      | attempt: 0 | changed: 630 | gc_id: WMSID.Host1.cfcd2 | dest: None
	01 | state: FAILED    | attempt: 1 | changed: 580 | gc_id: WMSID.Grid.c4ca4  | dest: ce1.fnal.gov/medium
	02 | state: SUCCESS   | attempt: 0 | changed: 532 | gc_id: WMSID.Grid1.c81e7 | dest: ce1.fnal.gov/medium
	03 | state: INIT      | attempt: 0 | changed: 486 | gc_id: WMSID.ARC.eccbc   | dest: ce2.gridka.de/test
	04 | state: SUBMITTED | attempt: 0 | changed: 442 | gc_id: WMSID.Host1.a87ff | dest: ce1.gridka.de/medium
	05 | state: DISABLED  | attempt: 2 | changed: 400 | gc_id: WMSID.Grid.e4da3  | dest: ce2.fnal.gov
	06 | state: READY     | attempt: 0 | changed: 360 | gc_id: WMSID.Grid1.16790 | dest: None
	07 | state: WAITING   | attempt: 1 | changed: 322 | gc_id: WMSID.ARC.8f14e   | dest: ce1.gridka.de/medium
	08 | state: QUEUED    | attempt: 0 | changed: 286 | gc_id: WMSID.Host1.c9f0f | dest: ce1.gridka.de/medium
	09 | state: ABORTED   | attempt: 0 | changed: 252 | gc_id: WMSID.Grid.45c48  | dest: ce2.gridka.de/test
	14 | state: DONE      | attempt: 0 | changed: 112 | gc_id: WMSID.Grid1.aab32 | dest: ce1.gridka.de/medium
	17 | state: INIT      | attempt: 2 | changed: 52  | gc_id: WMSID.Grid.70efd  | dest: ce2.fnal.gov
	20 | state: READY     | attempt: 0 | changed: 10  | gc_id: WMSID.Host1.98f13 | dest: ce1.gridka.de/medium
	21 | state: WAITING   | attempt: 0 | changed: 0   | gc_id: WMSID.Grid.3c59d  | dest: ce2.gridka.de/test
	22 | state: QUEUED    | attempt: 0 | changed: 8   | gc_id: WMSID.Grid1.b6d76 | dest: ce1.gridka.de/medium
	23 | state: ABORTED   | attempt: 2 | changed: 14  | gc_id: WMSID.ARC.37693   | dest: ce2.fnal.gov
	24 | state: RUNNING   | attempt: 0 | changed: 18  | gc_id: WMSID.Host1.1ff1d | dest: None
	25 | state: CANCEL    | attempt: 1 | changed: 20  | gc_id: WMSID.Grid.8e296  | dest: ce1.gridka.de/medium
	26 | state: UNKNOWN   | attempt: 0 | changed: 20  | gc_id: WMSID.Grid1.4e732 | dest: ce1.gridka.de/medium
	27 | state: CANCELLED | attempt: 0 | changed: 18  | gc_id: WMSID.ARC.02e74   | dest: ce2.gridka.de/test
	28 | state: DONE      | attempt: 0 | changed: 14  | gc_id: WMSID.Host1.33e75 | dest: ce1.gridka.de/medium
	29 | state: FAILED    | attempt: 2 | changed: 8   | gc_id: WMSID.Grid.6ea9a  | dest: ce2.fnal.gov
	30 | state: INIT      | attempt: 0 | changed: --- | gc_id: None              | dest: None
	31 | state: INIT      | attempt: 0 | changed: --- | gc_id: None              | dest: None
