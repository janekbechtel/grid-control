import ast, sys, get_file_list, linecache, json, itertools
...
enums_use_hash = {}
config_calls = []
for (fn, fnrel) in get_file_list.get_file_list(showExternal = False, showAux = False, showScript = False, showTestsuite = False, showTypes = ['py']):
	if 'scriptlets' in fn:
		continue
...
def join_config_locations(opt_first, *opt_list):
	if isinstance(opt_first, (list, tuple)):  # first option is a list - expand the first parameter
		if not opt_list:  # only first option -> clean and return
			return list(map(str.strip, opt_first))
		return list(itertools.chain(*map(lambda opt: join_config_locations(opt.strip(), *opt_list), opt_first)))
	if not opt_list:  # only first option -> clean and return
		return [opt_first.strip()]
	def do_join(opt):
		return opt_first + ' ' + opt
	return list(map(do_join, join_config_locations(*opt_list)))
...
for result in config_calls:
	def parse_option_spec(value):
		if isinstance(value, ast.Str):
			...
			args_list = []
			for parg in map(parse_option_spec, value.args):
				if isinstance(parg, (list, tuple)):
					args_list.append(map(lambda x: x.strip().strip('"').strip("'"), parg))
				else:
