...
def display_report(workflow):
	...
	report = Report.create_instance('BasicReport', workflow.job_manager.job_db, workflow.task)
	report._write_line = write_line
	report.show_report(workflow.job_manager.job_db)
...
def do_resync_test(settings, resync_time = 0, report = True):
	...
	def display():
		checkTask(workflow)
		print('____')
		display_jobs(workflow.job_manager.job_db)
		if report:
			print('____')
